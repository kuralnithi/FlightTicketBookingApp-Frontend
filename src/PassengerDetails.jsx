...
const PassengerDetails = () => {
 ...
 const [showModal, setShowModal] = useState(false);
  ...
  const {myflight,setMyflight,passengerDetails,setPassengerDetails,selectedSeats, setSelectedSeats,totalCost, setTotalcost } = useContext(myContext);
...
const handleSeatSelection = (seatNumber) => {
  ...
  if (index === -1 && selectedSeats.length < passengerCount) {
    ...
    const passengerIndex = passengerDetails.findIndex((passenger) => !passenger.seatNumber);
    if (passengerIndex !== -1) {
    ...
    const passengerIndex = passengerDetails.findIndex((passenger) => passenger.seatNumber === seatNumber);
    if (passengerIndex !== -1) {
      const updatedDetails = [...passengerDetails];
  ...
  const renderSeats = () => {
    ...
    for (let i = 1; i <= rows; i++) {
      ...
      groupSeats.push(<div key={i} className="seat-row">{rowSeats}</div>);
    }
    ...
    <div className="container-fluid text-center passengercontainer">
      ...
      {passengerDetails.map((passenger, index) => (
        <div key={index} className="form-control passenger-detailscard row container-fluid ">
          ...
          <input className='form-control col-md-3 col-12 mt-4 mx-1'
max={130}
            min={1}
             required
                  type="number"
...
<input
  ...
  onChange={(e) => handlePassengerDetailsChange(index, 'mobile', e.target.value)}
/>
...
<hr className='pshrline'/>
        <div className="total"><span>Total Price = ₹ {+myflight.price.slice(1) * passengerCount} </span>  </div>
      <button  className='btn payconbtn' onClick={handleConfirmPayment}>confirm payment</button> <br />    
      <button className='btn payconbtn' onClick={handleCancel}>back to main</button>    
      </div>
